...
9 & Forward/Futuro FX & En corto & oct-26 & 3.797 & COP/USD & 19.065.360 & USD & -- \\ 
...
7 & Bloomberg (FX Bid) \\
8 & Bloomberg (FX Bid) \\
9 & Interpolación a través de combinación lineal de tasa implícita entre la tasa en pesos resultante del forward de octubre y la del futuro de diciembre utilizando el swap de la SOFR para octubre (3,52\%) \\
...
&  & 19.065.360 & 3.797 & 0 & 72.395.118.287 & -19.065.360 \\
...
\end{table}
 ...
 Luego, para los meses entre marzo y agosto se utilizó el precio de los forwards del dólar obtenidos de la plataforma Bloomberg. Para obtener este precio, se tomo el punto medio entre el precio de Bid y Ask, considerandolo como la mejor aproximación posible a la Ley de un solo precio. Además, tomar un valor extremo en este rango entre el Bid y el Ask puede arrojar resultados contraintuitivos como tasas negativas. Una vez se tuvo este precio, se utilizó la Paridad Cubierta de Tasas de interés, o la formula de no arbitraje para forwards de divisas, utilizando tasas de interés continuas y constantes, con la tasa del dólar como la SOFR, o para ser más precisos los swaps de la tasa SOFR a cada uno de los plazos, disponibles en Bloomberg. Con esta aproximación, se despejó la tasa ímplicita en pesos. A continuación, la expresión matemática resultante del despeje.

\[
TLR_{t+h} = SOFR_{t+h} + \frac{ln\left(\frac{F_{t+h}}{TRM_t}\right)}{\left(\frac{\text{Días entre }_t\text{y}_h}{365}\right)}
\]

Dónde la tasa SOFR respresentá los precios medios de sus swaps, la TRM un valor de 3.665 y los días entre t y h se tomaron como la cantidad de días entre el 13 de febrero y el día del mes del vencimiento del forward encontrado \footnote{Si bien el ejercicio asume que las transacciones se dan el último día de cada mes, para el cálculo de la tasa se tomaron solamente los días hasta que se venciera el futuro encontrado para el mes en cuestión, dado que en las fuentes de información consultadas tendía a ofrecerse un solo vencimiento para cada mes}.\\

Este despeje se realizó hasta agosto, mes a partir del cual, no se encontraron contratos futuros o forwards disponibles, sino hasta diciembre a través de la plataforma de la Bolsa de Valores de Colombia. Para diciembre, se realizó el cálculo de la tasa de la misma manera descrita anteriormente. Para los meses entre septiembre y noviembre, se realizó una interpolación lineal en las tasas libres de riesgo en pesos, procedimiento que ya se había aplicado para encontrar el precio forward del dólar en octubre para terminar de conformar el portafolio en la primera tabla. Sin embargo, en ese ejercicio previo se habían interpolado las tasas de las operaciones Bid, en este caso se realiza la interpolación con el punto medio entre las tasas Bid y Ask.\\

A continuación, las tasas implícitas resultantes:\\

\begin{table}[H]
\centering
\caption{Forward, tasa SOFR e implícita en pesos (2026)}
\scriptsize
\renewcommand{\arraystretch}{1.2}
...
\toprule
\textbf{Mes} &
\textbf{Precio forward tasa de cambio} &
\textbf{Swap tasa SOFR} &
\textbf{Tasa implícita pesos} \\
\midrule

feb-26 & 3.665 & 3,66\% & 0,00\% \\
mar-26 & 3.678 & 3,66\% & 7,51\% \\
abr-26 & 3.703 & 3,66\% & 9,24\% \\
may-26 & 3.729 & 3,65\% & 10,24\% \\
jun-26 & 3.755 & 3,64\% & 10,71\% \\
jul-26 & 3.783 & 3,62\% & 10,92\% \\
ago-26 & 3.810 & 3,58\% & 11,06\% \\
sep-26 & 3.834 & 3,56\% & 11,16\% \\
oct-26 & 3.861 & 3,52\% & 11,26\% \\
nov-26 & 3.888 & 3,49\% & 11,36\% \\
dic-26 & 3.932 & 3,46\% & 11,46\% \\
...
oct-26 & 72,40 & 9,25 & 14,70 & 48,54 & 3.867 & 11,47\% & 44,74 \\
...
De esta manera, se concluye que, el valor completo en pesos de la utilidad en valor presente del 2026 es de: COP 125.574.473.264,33 (ciento veinticinco mil quinientos setenta y cuatro millones cuatrocientos setenta y tres mil doscientos sesenta y cuatro pesos).
    ...
    En el cuadro \ref{tab:parametros} se puede ver el resumen de los par\'ametros utilizados para calibrar el modelo y valorar las opciones requeridas. Adem\'as de los datos dados en el enunciado, se presenta la volatidad anualizada obtenida a partir de la serie historica de retornos para la acci\'on de Apple.


    \input{output/parameters_table}

    Para calibrar los par\'ametros $u$ y $d$ se utilizaron las siguientes formulas, derivadas a partir de la definici\'on de probabilidad neutral al riesgo y de la varianza de los retornos logar\'itmicos. El proceso matem\'atico mediante el cual se obtuvieron puede consultarse en el anexo.
    ...
    Una vez calculados los par\'ametros $u$ y $d$ se calculo el arbol binomial para la acci\'on de Booble, el cual se puede observar en el Cuadro \ref{tab:subyacente}, y sirve como insumo para el calculo de cada una de las opciones a continuaci\'on.

    \input{output/arbol_subyacente}
    
    \subsection{Valoraci\'on opci\'on put americana}

    Como primer ejercicio se valor\'o un opci\'on put am\'ericana, la cual permite a la parte larga ejercer su opci\'on de compra en cualquier momento entre la fecha de compra y la fecha de expiraci\'on.

    El cuadro \ref{tab:puntoa} muestra el arbol binomial recombinante para la valoraci\'on de dicha opci\'on.

    A partir de los precios del subyacente (acci\'on de Booble) y el \emph{strike} se calcularon los pagos finales
    
    \input{output/arbol_puntoa_ame}

    \subsection{Valoraci\'on opci\'on put con knockout}

    \input{output/arbol_puntob_eur}

    \subsection{Valoraci\'on \emph{chooser at-the-money}}

    Finalmente se valoro una opci\'on \emph{chooser at-the-money}, con strike $310$ y shout de $6$ meses. Este instrumento permite a la parte larga a escoger en un tiempo futuro (en este caso 6 meses) si la opci\'on adquirida es put o call. Dando as\'i un plaz

    \input{output/arbol_call_c}

    \input{output/arbol_put_c}
    
    \input{output/arbol_c}
